   ...
   stage('Deploy to EC2 IIS') {
            steps {
                withCredentials([usernamePassword(credentialsId: 'ec2-iis-credentials', passwordVariable: 'EC2_PASS', usernameVariable: 'EC2_USER')]) {
                    bat """
                    @echo off
                    echo "Streaming compiled files over port 8172 using explicit authorization..."
                    
                    "C:\\Program Files\\IIS\\Microsoft Web Deploy V3\\msdeploy.exe" ^
                    -source:contentPath="%WORKSPACE%\\%PUBLISH_DIR%" ^
                    -dest:contentPath="CustomerHub",computerName="https://16.171.14.84:8172/msdeploy.axd?site=CustomerHub",username="%EC2_USER%",password="%EC2_PASS%",authType="Basic" ^
                    -verb:sync ^
                    -allowUntrusted
                    """
                }
